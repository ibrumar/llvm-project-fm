...
static bool matchIntrinsicCalls(Intrinsic::ID ID, const CallTy *CI1,
                                const CallTy *CI2) {
  Intrinsic::ID ID1;
  Intrinsic::ID ID2;
...
static bool matchGetElementPtrInsts(const GetElementPtrInst *GEP1,
  ...
  for (unsigned i = 1; i < Idxs1.size(); i++) {
    ...
    Ty1 = dyn_cast<CompositeType>(Ty1)->getTypeAtIndex(V1);
    Ty2 = dyn_cast<CompositeType>(Ty2)->getTypeAtIndex(V2);
    if (Ty1 != Ty2)
      return false;
  }
  return true;
...
template<typename CallTy>
static bool matchCallInsts(const CallTy *CI1, const CallTy *CI2) {
...
public:
  ...
  FingerprintMH(T owner, SearchStrategy &searchStrategy) : _footprint(searchStrategy.item_footprint()) {
	...
	if (TerminatorInst *TI = dyn_cast<TerminatorInst>(&I)) {
            OpcodeFreq[0] += TI->getNumSuccessors();
	}
        //if (I.isTerminator())
...
template<typename T>
static bool SortCmpItem(T &item1, T &item2) {
        return item1.FP.magnitude > item2.FP.magnitude;
}

template<typename T>
static bool SortCmpMatchGT(T &match1, T &match2) {
                          return match1.Distance > match2.Distance;
         }

template<typename T>
static bool SortCmpMatchLT(T &match1, T &match2) {
                          return match1.Distance < match2.Distance;
         }
...
public:
  ...
  T next_candidate() override {
    if (!initialized) {
      candidates.sort(SortCmpItem<MatcherEntry>);
      //sort(candidates.begin(), candidates.end(), SortCmpItem);
  ...
  void update_matches(MatcherIt it) {
    ...
    if (ExplorationThreshold < matches.size()) {
      std::partial_sort(matches.begin(), matches.begin() + ExplorationThreshold,
                        matches.end(), SortCmpMatchLT< MatchInfo<T> >);
                        /*
      ...
      std::sort(matches.begin(), matches.end(),SortCmpMatchGT< MatchInfo<T> >);
                /*
                [&](auto &match1, auto &match2) -> bool {
                  return match1.Distance > match2.Distance;
                });*/
...
//#endif
  ...
  */
  ...
  auto Gen = [&](SALSSACodeGen<Function::BasicBlockListType> &CG) {
...
template<typename CallTy>
void UpdateFn(CallTy CI, Function *F, FunctionMergeResult &MFR,
                                      const FunctionMergingOptions &Options, const DataLayout *DL, Type *IntPtrTy) {
  ...
  Function *MergedF = MFR.getMergedFunction();
    ...
    Value *CastedV = nullptr;
    if (CI->getOpcode() == Instruction::Call) {
      CallInst *NewCB = (CallInst *)Builder.CreateCall(MergedF->getFunctionType(),
                                             MergedF, args);
      ...
      //InvokeInst *NewCB = (InvokeInst *)Builder.CreateInvoke(MergedF->getFunctionType(),
      //                                           MergedF, II->getNormalDest(),
      //                                           II->getUnwindDest(), args);
      InvokeInst *NewCB = (InvokeInst *)InvokeInst::Create(MergedF->getFunctionType(),
                                                 MergedF, II->getNormalDest(),
                                                 II->getUnwindDest(), args,"",CI);
      // MergedF->dump();
    ...
    if (!F->getReturnType()->isVoidTy()) {
      if (MFR.needUnifiedReturn()) {
        Value *AddrCI = Builder.CreateAlloca(CastedV->getType());
        Builder.CreateStore(CastedV, AddrCI);
        Value *CastedAddr = Builder.CreatePointerCast(
            AddrCI,
            PointerType::get(F->getReturnType(), DL->getAllocaAddrSpace()));
        CastedV = Builder.CreateLoad(F->getReturnType(), CastedAddr);
      } else {
        CastedV = createCastIfNeeded(CastedV, F->getReturnType(), Builder,
                                     IntPtrTy, Options);
      }
    }
...
bool FunctionMerger::replaceCallsWith(Function *F, FunctionMergeResult &MFR,
  ...
  for (Instruction *I : Calls) {
    if (CallInst *CI = dyn_cast<CallInst>(I))
      UpdateFn(CI,F,MFR,Options,DL,IntPtrTy);
    else if (InvokeInst *II = dyn_cast<InvokeInst>(I))
      UpdateFn(II,F,MFR,Options,DL,IntPtrTy);
  }
...
bool FunctionMerging::runImpl(
  ...
  //  matcher = llvm::make_unique<MatcherLSH<Function *>>(FM, Options, LSHRows, LSHBands); errs() << "LSH MH\n"; }
  //else if (EnableLSH && linearScan){
  //  matcher = llvm::make_unique<MatcherFQ<Function *, FingerprintMH>>(FM, Options); errs() << "LIN SCAN MH\n"; }
  //else
  { matcher = llvm::make_unique<MatcherFQ<Function *>>(FM, Options); errs() << "LIN SCAN FP\n";  }
    ...
    if (Entry.match()) {
      ...
      std::string BBName =
          (I1 == nullptr) ? "m.label.bb"
                          : (isa<TerminatorInst>(I1) ? "m.term.bb" : "m.inst.bb");
