...
std::map< std::string, double > FunctionMerging::bbProfiles;
std::map< std::string, double > FunctionMerging::opLuts;
std::set< std::string> FunctionMerging::BlackList;

std::map< std::string, double>  FunctionMerging::realAreaPerFunc;
std::map< BasicBlock *, double > FunctionMerging::mergedBBProfiles;
std::map< Function *, double > FunctionMerging::funcTimeSize;
...
static cl::opt<double> LoadImbalanceThreshold( //orders of magnitude of differenence are tollerable
    "func-merging-li", cl::init(999999.0), cl::Hidden,
...
static cl::opt<bool> LatencyMerges("func-merging-latency-merges", cl::init(false), cl::Hidden,
...
static cl::opt<bool>
    RunBruteForceExploration("func-merging-oracle", cl::init(false), cl::Hidden,
...
static unsigned int getAreaEstimOld(Instruction *Inst) {
    if (FunctionMerging::opLuts.find(Inst->getOpcodeName()) == FunctionMerging::opLuts.end())
        errs() << "For some reason the opcode " << Inst->getOpcode() << " could not be found\n";
    return FunctionMerging::opLuts[Inst->getOpcodeName()];
}
...
static double estimateFunctionTimeOvh(Function &F, TargetTransformInfo *TTI, double &timeSize) {
  ...
  for (auto &BB : F) {
    ...
    if (FunctionMerging::mergedBBProfiles.find(&BB) != FunctionMerging::mergedBBProfiles.end()) {
      errs() << "Could find bb " << bbId << " which belongs to func " << F.getName().str() << "\n";
      functionProfile = FunctionMerging::mergedBBProfiles[&BB];
    }
    else
...
static void estimateNumberOfCallsForFunc(Function &F) {
    ...
    //
    //if (FunctionMerging::bbProfiles.find(bbId) != FunctionMerging::bbProfiles.end()) {
    //    FunctionMerging::bbProfiles[bbId]/numInstr;
    //}
        ...
        for (auto &BB : F_temp) {
            ...
            for (auto &I : BB) {
                ...
                if (CI = dyn_cast<CallInst>(&I)) {
                    ...
                    if (&F == CI->getCalledFunction() and FunctionMerging::bbProfiles.find(bbId) != FunctionMerging::bbProfiles.end()) {
                        double profile = FunctionMerging::bbProfiles[bbId]/numInstr;
...
static double estimateFunctionLatencyOrArea(Function &F, EstimationType estTy) {
  ...
  for (BasicBlock &BB : F){
    ...
    if (estTy == HW_LATENCY_ACCEL or estTy == SW_LATENCY_ACCEL)
      if ((FunctionMerging::bbProfiles.find(GetValueName(&F) + "::" + BB.getName().str()) != FunctionMerging::bbProfiles.end())) 
          functionProfile = FunctionMerging::bbProfiles[GetValueName(&F) + "::" + BB.getName().str()];
      else if (FunctionMerging::mergedBBProfiles.find(&BB) != FunctionMerging::mergedBBProfiles.end())
          functionProfile = FunctionMerging::mergedBBProfiles[&BB];
      else
...
static double estimateFunctionDifference(Function &FMerged, Function &FInput1, Function &FInput2) {
  ...
  for (auto it = FunctionMerging::opLuts.begin(); it != FunctionMerging::opLuts.end(); ++it)
    opCounts[it->first] = 0;
  ...
  errs() << "\n";
  lutDifference += FunctionMerging::opLuts["intercept"];
        //FunctionMerging::opLuts[Inst->getOpcodeName()];
...
static double estimateFunctionSize(Function &F, TargetTransformInfo *TTI, double &timeSize) {
  ...
  //if (FunctionMerging::bbProfiles.empty()) {
  ...
  for (auto &BB : F) {
    ...
    } else if (FunctionMerging::bbProfiles.find(bbId) != FunctionMerging::bbProfiles.end()) {
      functionProfile = FunctionMerging::bbProfiles[bbId];
      //errs() << "The function profile you've found for bb " << bbId << " is in bbProfiles " << functionProfile << "\n";
    
    } else if (FunctionMerging::bbProfiles.empty()) {
      functionProfile = 1.0;
  ...
  //if (LatencyMerges and not FunctionMerging::bbProfiles.empty()) {
...
bool llvm::matchIntrinsicCalls(Intrinsic::ID ID, const CallInst *CI1,
                                const CallInst *CI2) {
  Intrinsic::ID ID1;
  Intrinsic::ID ID2;
...
bool llvm::matchLandingPad(LandingPadInst *LP1, LandingPadInst *LP2) {
...
bool llvm::match(Instruction *I1, Instruction *I2) {
...
bool llvm::match(Value *V1, Value *V2) {
...
bool llvm::match(std::vector<Value *> &F1, std::vector<Value *> &F2,
                  unsigned i, unsigned j) {
  return match(F1[i], F2[j]);
}

bool llvm::match(SmallVectorImpl<Value *> &F1, SmallVectorImpl<Value *> &F2,
                  unsigned i, unsigned j) {
  return match(F1[i], F2[j]);
}
...
static void setDynamicProfiling(BasicBlock *MergedResBB, BasicBlock *BB1, BasicBlock *BB2) {
    ...
    if (not FunctionMerging::bbProfiles.empty()) {
        ...
        if (FunctionMerging::bbProfiles.find(bb1Name) != FunctionMerging::bbProfiles.end()) {
            mergedResProfile = FunctionMerging::bbProfiles[bb1Name];
        }

        if (FunctionMerging::bbProfiles.find(bb2Name) != FunctionMerging::bbProfiles.end()) {
            mergedResProfile += FunctionMerging::bbProfiles[bb2Name];
        }

        FunctionMerging::bbProfiles[GetValueName(MergedResBB->getParent()) + "::" + MergedResBB->getName().str()] = mergedResProfile;
        FunctionMerging::mergedBBProfiles[MergedResBB] = mergedResProfile;
    }
}
  ...
  for (auto Pair : AlignedInsts) {
    if (Pair.first != nullptr && Pair.second != nullptr) {
      ...
      if (isa<BasicBlock>(Pair.first)) {
        ...
        double profile_bb1 = FunctionMerging::bbProfiles[GetValueName(F1) + "::" + BB1->getName().str()];
        double profile_bb2 = FunctionMerging::bbProfiles[GetValueName(F2) + "::" + BB2->getName().str()];
        ...
        FunctionMerging::bbProfiles[GetValueName(NewBB->getParent()) + "::" + NewBB->getName().str()] = profile_bb1 + profile_bb2;
        FunctionMerging::mergedBBProfiles[NewBB] = profile_bb1 + profile_bb2;
      ...
      if (isa<BasicBlock>(V)) {
        ...
        if (not FunctionMerging::bbProfiles.empty()) {
            double profile_bb = FunctionMerging::bbProfiles[GetValueName(F) + "::" + BB->getName().str()];
            FunctionMerging::bbProfiles[GetValueName(F) + "::" + NewBB->getName().str()] = profile_bb;
            FunctionMerging::mergedBBProfiles[NewBB] = profile_bb;
        }
        VMap[BB] = NewBB;
    ...
    if (Pair.first != nullptr && Pair.second != nullptr) {
      ...
      if (isa<BasicBlock>(Pair.first)) {
        ...
        if (MergedBB == nullptr) {
          ...
          //double profile_bb = FunctionMerging::bbProfiles[my_f2->getName().str() + "::" + bb_src2->getName().str()];
          //FunctionMerging::bbProfiles[MergedBB->getName().str() + "::" + MergedBB->getName().str()] = profile_bb;
          //FunctionMerging::mergedBBProfiles[MergedBB] = profile_bb;
...
*/
      ...
      if (MergedBB != nullptr) {
        ...
        if (not FunctionMerging::bbProfiles.empty()) {
            ...
            if (FunctionMerging::bbProfiles.find(bb1Name) != FunctionMerging::bbProfiles.end()) {
                FunctionMerging::mergedBBProfiles[NewBB1] = FunctionMerging::bbProfiles[bb1Name];
                FunctionMerging::bbProfiles[GetValueName(NewBB1->getParent()) + "::" + NewBB1->getName().str()] = FunctionMerging::bbProfiles[bb1Name];
            } else
                errs() << "Could not find " + bb1Name + " in bbProfiles for Nonalig1.\n"; 
            if (FunctionMerging::bbProfiles.find(bb2Name) != FunctionMerging::bbProfiles.end()){
                FunctionMerging::mergedBBProfiles[NewBB2] = FunctionMerging::bbProfiles[bb2Name];
                FunctionMerging::bbProfiles[GetValueName(NewBB2->getParent()) + "::" + NewBB2->getName().str()] = FunctionMerging::bbProfiles[bb2Name];
            } else
...
bool FunctionMerging::shouldPreserveGV(const GlobalValue &GV) {
...
void FunctionMerging::LoadRealFuncArea(const char *Filename) {
  // Load the BlockFile...
  ...
  while (In) {
    ...
    if (!FuncName.empty()) {
      FunctionMerging::realAreaPerFunc[FuncName] = funcArea;
      //bbProfiles.push_back(BasicBlockDynProfile(FunctionName, BlockName, numInstructions));
    }
  }
}

void FunctionMerging::LoadAreaModel(const char *Filename) {
  // Load the BlockFile...
  std::ifstream In(Filename);
  if (!In.good()) {
    errs() << "WARNING: Function merging couldn't load file '" << Filename
  ...
  while (In) {
    ...
    if (!OpName.empty()) {
      FunctionMerging::opLuts[OpName] = lutsPerOp;
      //bbProfiles.push_back(BasicBlockDynProfile(FunctionName, BlockName, numInstructions));
    }
  }
}

void FunctionMerging::LoadFile(const char *Filename) {
  // Load the BlockFile...
  std::ifstream In(Filename);
  ...
  while (In) {
    ...
    if (!BlockName.empty()) {
      ...
      FunctionMerging::bbProfiles["@" + FunctionName + "::" + BlockName] = percAppInstructions;
      //bbProfiles.push_back(BasicBlockDynProfile(FunctionName, BlockName, numInstructions));
    }
  }
}
...
void FunctionMerging::LoadBlacklistFile(const char *Filename) {
  // Load the BlockFile...
  std::ifstream In(Filename);
  if (!In.good()) {
...
static bool isProfitableAccelMerge(Function &F1, Function &F2, Function &F12) {
    ...
    if (not FunctionMerging::bbProfiles.empty() and canFind(F1, hwLatAtMergeTime, f1_hw) and canFind(F1, swLatAtMergeTime, f1_sw) 
...
static void printBBsWithTheirDynProf(Function *func) {
    ...
    for (auto &BB : *func) {
        ...
        if (FunctionMerging::mergedBBProfiles.find(&BB) != FunctionMerging::mergedBBProfiles.end())
            errs() << GetValueName(&BB) << " : " << FunctionMerging::mergedBBProfiles[&BB] << " a merged bb\n";
        else if (FunctionMerging::bbProfiles.find(bbProfilesKey) != FunctionMerging::bbProfiles.end())
            errs() << GetValueName(&BB) << " : " << FunctionMerging::bbProfiles[bbProfilesKey] << " a nonmerged bb\n";
        else
...
bool FunctionMerging::runOnModule(Module &M) {
  ...
  if (std::string(DynamicProfileFile.c_str()) != "filename" and std::string(DynamicProfileFile.c_str()) != "") {
    ...
    FunctionMerging::LoadFile(DynamicProfileFile.c_str());
  }
    ...
    while (!Rank.empty()) {
      ...
      if (Result.MergedFunc) {
        ...
        if ( BlackList.count( std::string((*Result.MergedFunc).getName().str()) )) {
            ...
            SizeF1 = 0;
            SizeF2 = 0;
        } else if (FunctionMerging::realAreaPerFunc.find((*Result.MergedFunc).getName().str()) != FunctionMerging::realAreaPerFunc.end()) {
            SizeF12 = FunctionMerging::realAreaPerFunc[(*Result.MergedFunc).getName().str()];
            SizeF1 = FunctionMerging::realAreaPerFunc[F1->getName().str()];
            SizeF2 = FunctionMerging::realAreaPerFunc[F2->getName().str()];
